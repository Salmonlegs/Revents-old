...
export default function EventFeedItem({ post }) {
	...
	switch (post.code) {
		...
		case 'left-event':
			summary = (
				<>
					<Link to={`/profile/${post.userUid}`}>{post.displayName}</Link> has cancelled their place
					on <Link to={`/events${post.eventId}`}>{post.title}</Link>
				</>
			);
			break;
		default:
	...
	return (
		<Feed>
			<Feed.Event>
				<Feed.Label style={{ display: 'inline-block' }} image={post.photoURL} />
				<Feed.Content>
